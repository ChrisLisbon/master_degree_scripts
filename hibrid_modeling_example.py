...
def nemo_domain_model(fitted_model: any, idx: np.array, predict_data: np.array, params: dict):
    """
    Function for physical model data returning
    """
    norm = params.get('norm')
    dif = params.get('dif')
    project_root_path = str(fedot_project_root())
    df = pd.read_csv(os.path.join(project_root_path, 'cases/data/arctic/nemo_multi_ts.csv'))
    time_series = df['56_86'].to_numpy()
    result = np.array((time_series * dif) - norm)
    result = result[-60:].reshape(1, -1)
    return result, 'table'


def prepare_time_series(time_series, forecast_length):
    """
    Function for time series transformation in InputData
    """
...
def get_initial_pipeline():
    ...
    lagged_node2 = PrimaryNode('lagged')
    custom_node = SecondaryNode('custom', nodes_from=[lagged_node2])
    custom_node.custom_params = {"norm": 0.5, "dif": 0.9, 'model_predict': nemo_domain_model}

    node_final = SecondaryNode('ridge', nodes_from=[ridge_node1, custom_node])
...
def get_available_operations():
    ...
    primary_operations = ['ar', 'smoothing', 'diff_filter', 'gaussian_filter']
    secondary_operations = ['lagged', 'ridge', 'lasso', 'linear', 'custom']
...
def compose_pipeline(pipeline, train_data, task, custom_params):
    ...
    composer_requirements = PipelineComposerRequirements(
        ...
        secondary=secondary_operations, max_arity=4,
...
def tune_pipeline(obtained_pipeline, train_data, task):
    ...
    custom_search_space = {'custom': {'norm': (hp.uniform, [0, 2]),
                                      'dif': (hp.uniform, [-2, 2]),
...
def run_hybrid_modeling(forecast_length):
    ...
    custom_params = {"norm": 0.5, "dif": 0.9, 'model_predict': nemo_domain_model}
